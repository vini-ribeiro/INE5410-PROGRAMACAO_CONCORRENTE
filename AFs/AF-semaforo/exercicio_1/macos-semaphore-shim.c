...
int ine5410_sem_destroy(ine5410_sem_t *sem) {
    ...
    if (code != 0) {
        printf("[validator] [sem_destroy no Mac OS] Erro ao fechar semáforo %s "
               "(sem_t* = %p, sem_t.ptr = %p): %s",
               name, sem, sem->ptr, strerror(old_errno));
        fflush(stdout);
    }
    errno = old_errno;
    return code;
...
int ine5410_sem_timedwait(ine5410_sem_t *sem, const struct timespec *ts) {
    ...
    size_t n_loops =
        (double) ts->tv_sec / 0.050 + (double) ts->tv_nsec / ns_step;
    int err = EAGAIN;
    for (size_t i = 0; i < n_loops && (err = ine5410_sem_trywait(sem)) != 0;
         ++i) {
